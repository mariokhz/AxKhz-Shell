...
class ConfigBase(dict):
	...
	system_location = (os.path.join(os.path.dirname(os.path.abspath(__file__)), "cavalcade"),)
	path = os.path.expanduser("~/.config/Ax-Shell/config/cavalcade")
...
class MainConfig(ConfigBase):
	...
	def _set_fallback_bg(self):
		self["image"]["default"] = os.path.join(os.path.dirname(self.defconfig), "")

	def write_data(self):
		super().write_data()
		self.save_data()

class MainConfigIn(ConfigBase):
	"""Main application config manager"""
	def __init__(self):
		super().__init__(
			"main_in.ini", dict(
